...
use crate::{
    board::Board,
    engine::defs::{HashTable, PerftData},
...
pub fn run(hash_table: Arc<Mutex<HashTable<PerftData>>>, hash_use: bool) {
    ...
    while (test_nr < number_of_tests) && (result == 0) {
        ...
        while index < test_data.len() && (result == 0) {
            ...
            if result == 0 {
                ...
                let now = Instant::now();
                let found_ln =
                    perft::perft(&mut board, depth, &move_generator, &hash_table, hash_use);
