            ...
            while !quit {
                ...
                if new_report.is_valid() {
                    ...
                    *t_last_report.lock().expect(ErrFatal::LOCK) = unknown;
                    ...
                    // Update the screen.
                    Uci::update(&t_last_report, &t_board);
                }
...
impl Uci {
    fn update(last_report: &Arc<Mutex<CommReport>>, board: &Arc<Mutex<Board>>) {
        match *last_report.lock().expect(ErrFatal::LOCK) {
            ...
            | CommReport::Uci(UciReport::Move(_))
            | CommReport::Uci(UciReport::Takeback) => {
                Uci::print_position(board);
                Uci::print_prompt();
            }
            _ => Uci::print_prompt(),
        }
    ...
    fn create_report(input: &str) -> CommReport {
        ...
        match &i[..] {
            ...
            "evaluate" | "e" => CommReport::Uci(UciReport::Evaluate),
            "takeback" | "t" => CommReport::Uci(UciReport::Takeback),
            "quit" | "q" => CommReport::General(GeneralReport::Quit),
            "exit" | "x" => CommReport::General(GeneralReport::Quit),
            _ => CommReport::Uci(UciReport::Move(i)),
        }
    }
}
...
impl Uci {
    ...
    fn print_help() {
        ...
        println!("{}", "=".repeat(Uci::HELP_UNDERLINE));
        for line in Uci::HELP.iter() {
